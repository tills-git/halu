...
class BatchList_view():
    ...
    def __init__(self, mvclog, mvclogname, responsedict, sqldict, requestdict):
        try:
            ...
            self.mvclog.debug(self.mvclogname, 'BatchList_view init start')
            ...
            self.mvclog.debug(self.mvclogname, 'BatchList_view init exception message : {e}')

        finally:
            self.mvclog.debug(self.mvclogname, 'BatchList_view init end')

    def getMaxPageCountOfBatchList(self, idName):
        """
        ...
        status : str
            'OK' 固定
        """
        self.mvclog.debug(self.mvclogname, 'BatchList_view getMaxPageCountOfBatchList start')

        tempObj = Pagination_view(self.responsedict, self.mvclog, self.mvclogname)
        status  = tempObj.getMaxPageCount(idName)

        self.mvclog.debug(self.mvclogname, 'BatchList_view getMaxPageCountOfBatchList end')
        return status
